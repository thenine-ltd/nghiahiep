...
class NorebroImporter {
	...
	public function create_plugin_page() {
		$plugin_page_setup = apply_filters( 'pt-ocdi/plugin_page_setup', array(
				...
				'page_title'  => esc_html__( 'Norebro Importer' , 'norebro-importer' ),
		...
		if ( $this->plugin_page === $hook ) {
			...
			wp_localize_script( 'ocdi-main-js', 'ocdi',
				array(
					...
					'dashboard_link' => get_admin_url( null, 'admin.php?page=norebro_hub' ),
					'texts' => array(
						'missing_preview_image' => esc_html__( 'No preview image defined for this import.', 'norebro-importer' ),
						'dialog_title' => esc_html__( 'Demo Import', 'norebro-importer' ),
						'dialog_no' => esc_html__( 'Cancel', 'norebro-importer' ),
						'dialog_close' => esc_html__( 'Close', 'norebro-importer' ),
						'dialog_activate' => esc_html__( 'Activate', 'norebro-importer' ),
						'dialog_yes' => esc_html__( 'Yes, import!', 'norebro-importer' ),
						'install_globals' => esc_html__( 'Install Theme Settings', 'norebro-importer' ),
					),
					'dialog_options' => apply_filters( 'pt-ocdi/confirmation_dialog_options', [] )
				)
			);

			wp_enqueue_style( 'ocdi-main-css', PT_OCDI_URL . 'assets/css/norebro-importer.css', [], PT_OCDI_VERSION );
		}
	}
	...
	public function import_demo_data_ajax_callback() {
		...
		if ( ! $use_existing_importer_data ) {
			...
			if ( ! empty( $_FILES ) ) { // Using manual file uploads?
				...
				$this->import_files[ $this->selected_index ]['import_file_name'] = esc_html__( 'Manually uploaded files', 'norebro-importer' );
			}
			elseif ( ! empty( $this->import_files[ $this->selected_index ] ) ) { // Use predefined import files from wp filter: pt-ocdi/import_files.
				...
				if ( is_wp_error( $this->selected_import_files ) ) {
					...
					Helpers::log_error_and_send_ajax_response(
						...
						$this->log_file_path,
						esc_html__( 'Downloaded files', 'norebro-importer' )
					);
				}
				...
				$log_added = Helpers::append_to_file(
					sprintf(
						__( 'The import files for: %s were successfully downloaded!', 'norebro-importer' ),
				...
				wp_send_json( esc_html__( 'No import files specified!', 'norebro-importer' ) );
			}
		}
...
//
		...
		$native_parser = new \Norebro_WP_Import([
...
//
	...
	public function import_exec( $import_file_path ) {
		...
		if ( ! empty( $import_file_path ) ) {
			if ( @file_exists( $import_file_path ) ) {
				...
				include PT_OCDI_PATH . 'inc/NorebroDefaultParams.php';
				global $wpdb;
	...
	private function final_response() {
		...
		if (class_exists('NorebroOptionsCache')) {
			\NorebroOptionsCache::flush();
		}
		...
		//if ( empty( $this->frontend_error_messages ) ) {
			...
			$response['message'] .= sprintf(
				__( '%1$s%3$sThat\'s it, all done!%4$s%2$sThe demo import has finished. Please check the demo and make sure that everything has imported correctly.', 'norebro-importer' ),
		...
		/*}
			...
			$response['message'] .= sprintf(
				__( '%1$sThe demo import has finished, but there were some import errors.%2$sMore details about the errors can be found in this %3$s%5$slog file%6$s%4$s%7$s', 'norebro-importer' ),
	...
	public function load_textdomain() {
		load_plugin_textdomain( 'norebro-importer', false, plugin_basename( dirname( __FILE__ ) ) . '/languages' );
	}
