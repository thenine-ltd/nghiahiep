	...
	if ( class_exists( 'OCDI_Plugin' ) ) {
		...
		?>
			<div class="notice notice-error">
				<p>
					<strong><?php esc_html_e( '"Norebro Importer" plugin conflicts with "One Click Demo Import" plugin. Activation of both plugins is not possible at the same time.', 'norebro-importer' ); ?></strong>
					<br>
					<?php _e( 'Please deactivate "One Click Demo Import" plugin in order to use "Norebro Importer".', 'norebro-importer' ); ?>
				</p>
			</div>
		<?php
		}
		...
		class OCDI_Plugin {
			...
			public function __construct() {
				...
				if ( version_compare( phpversion(), '5.3.2', '<' ) ) {
					...
					$pt_one_click_demo_import = OCDI\NorebroImporter::get_instance();
				}
			}
