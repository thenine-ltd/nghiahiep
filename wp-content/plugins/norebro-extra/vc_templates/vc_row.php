...
// Norebro modification

$side_background_title_uniqid = uniqid('norebro-custom-');

$side_background_title_settings = NorebroExtraParser::VC_color_to_CSS( $side_background_title_color, 'color:{{color}};' );
$side_background_title_settings .= NorebroExtraParser::VC_typo_to_CSS( $side_background_title_typo );

NorebroExtraParser::VC_typo_custom_font( $side_background_title_typo );
...
if ( $use_through_lines ) {
	$norebro_lines_class = 'vc-bg-lines';
	switch ( $through_lines_style ) {
		case 'light':
			$norebro_lines_class .= ' light';
			break;
		case 'dark':
		default:
			$norebro_lines_class .= ' dark';
			break;
	}

	$output .= '<div class="' . $norebro_lines_class . '">';
	for( $i = 0; $i < 7; $i++ ){
		$output .= '<div></div>';
	}
	$output .= '</div>';
