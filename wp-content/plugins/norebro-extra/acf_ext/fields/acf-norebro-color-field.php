...
if( ! class_exists( 'acf_field_norebro_color' ) ) :


class acf_field_norebro_color extends acf_field {

	function __construct( $settings ) {

		$this->name = 'norebro_color';
		/*
		*  label (string) Multiple words, can include spaces, visible when selecting a field type
		*/
		$this->label = esc_html__( 'Norebro Color', 'norebro-extra' );
		/*
	...
	function render_field( $field ) {
		...
		*/
		...
		$uniqid = uniqid( 'norebro-color' );
?>

		<div class="norebro-acf-color-field-content" data-uniqid="<?php echo $uniqid; ?>">
...
<?php
	...
	function input_admin_enqueue_scripts() {
		...
		wp_register_script( 'acf-input-norebro-color', "{$url}assets/js/input.js", array( 'acf-input' ), $version );
		wp_enqueue_script('acf-input-norebro-color');
...
// initialize
new acf_field_norebro_color( $this->settings );
