!function($) {
    ...
    function norebroTypographySerialize($block, $hidden_input) {
        ...
        var uppercase = $block.find('input[data-target="uppercase"]').prop('checked');
        ...
        if (uppercase) { serialize_string += 'uppercase~1||'; }
    ...
    $('#vc_ui-panel-edit-element').on('change', '.norebro_extra_typography_block input, .norebro_extra_typography_block select', function(e){
        var $closest = $(this).closest('.norebro_extra_typography_block');
        var $value_hidden_input = $closest.find('.wpb_vc_param_value');
        norebroTypographySerialize( $closest, $value_hidden_input );
    });


    $('#vc_ui-panel-edit-element').on('change', '.norebro_extra_typography_block input[data-target="use-custom-font"]', function(e){
        if ($(this).prop('checked')) {
            $(this).closest('.norebro_extra_typography_block').find('.custom-font-panel').show();
        } else {
            $(this).closest('.norebro_extra_typography_block').find('.custom-font-panel').hide();
        }
    });
}(window.jQuery);