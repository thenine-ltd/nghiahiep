jQuery(function ($) {
    ...
    if ($('.clb-hub').length) {
        ...
        // Activation button
        $('#norebro-activate-theme-license').on('click', function () {
            if ($(this).attr('loading')) {
                return false;
            }

			const popupWindow = window.open(
				'https://demo.clbthemes.com/v1/activate?product=norebro',
				'Theme activation',
			...
			const messageListener = (e) => {
				...
				jQuery.post(window.ajaxurl, {
					action: 'norebro_save_license_code',
					license: licenseJSON
				}, () => {
        ...
        $('#norebro-remove-theme-license').on('click', function () {
            if ($(this).attr('loading')) {
                return false;
            }
            ...
            jQuery.post(window.ajaxurl, { 'action': 'norebro_remove_license_code' }, () => {
                window.location.reload();
            });
        });
        ...
        jQuery('#sync-languages-action').on('click', function () {
            ...
            jQuery.post(window.ajaxurl, {
                'action': 'norebro_sync_settings_with_main_lang',
        ...
        jQuery('#export_theme_settings').on('click', function(e) {
            ...
            jQuery.post(window.ajaxurl, {
                action: 'norebro_export_theme_settings'
            }, response => {
                const link = document.createElement('a');
                const file = new Blob([response], { type: 'application/json' });
                link.href = URL.createObjectURL(file);
                link.download = `norebro_setting_export_${new Date().toISOString()}.json`;
                link.click();
        ...
        jQuery('#import_theme_settings').on('submit', function(e) {
            ...
            data.append('action', 'norebro_import_theme_settings');
        ...
        jQuery('#reset_theme_settings').on('click', function(e) {
            ...
            if (confirm('Are you sure you want to reset your settings? This process is irreversible unless you\'ve made a backup.')) {
                jQuery.post(window.ajaxurl, {
                    action: 'norebro_reset_theme_settings'
                }, () => {
                    window.location.reload();
                })
            }
        ...
        jQuery.post(window.ajaxurl, { 'action': 'norebro_check_last_version' }, (response) => {
            let data = JSON.parse(response);

            if (versionCompare(data.current, data.actual) < 0) {
                $('#norebro-version-table-placeholder').text(data.actual);
                $('#norebro-version-table-value').find('mark').removeClass('yes').addClass('no');
                $('#norebro-version-table-value').find('.norebro-new-version-available').show();
            }
        });
    ...
    $(".clb-steps-item:first-child").on({
        mouseenter: function () {
            $('.toplevel_page_norebro_hub').addClass('visible');
        },
        mouseleave: function () {
            $('.toplevel_page_norebro_hub').removeClass('visible');
        }
    });
});
